...
# ========================= Base DAG and data generation (updated to A-B1-G1-Y & incoming edges from G2) =========================
...
def generate_random_coefficients(G=None, seed=None):
    if seed is not None:
        np.random.seed(seed)
    coefficients = {}
    edges = ['A_B1', 'B1_G1', 'G1_Y', 'G2_A', 'G2_B1', 'G2_Y', 'A_Y']
    for edge in edges:
...
def generate_data_from_dag(G, coefficients, n_samples=1000, seed=0, nonlinear_func=None):
    ...
    # Exogenous variable
    data['G2'] = np.random.normal(0, 1, n_samples)
...
# ========================= Estimator (AIPW + two marginal integrations; kept consistent with your original version) =========================
def _unique_and_weights(keys_series: pd.Series):
    """Stable key order + one-to-one weights; avoid default sorting that differs across platforms when ties occur."""
    if not isinstance(keys_series, pd.Series) or len(keys_series) == 0:
        return [()], {(): 1.0}
    vc = keys_series.value_counts(dropna=False, sort=False)
    def _safe_tuple(x):
        return x if isinstance(x, tuple) else (x,)
    # Stable sorting: length first, then lexicographic order (no float coercion)
...
def estimate_marginal_effect(data, treatment, outcome, vas, all_mediators,
    ...
    # Reproducible hooks (safe in non-Ray environments)
    ...
    # 3) Block-wise computation of E_{Z2}[mu] and E_{Z1}[mu]
    EZ2_mu1, EZ2_mu0 = {}, {}
...
def generate_single_coeff_tuple(G_edges, seed, nonlinear_funcs):
    ...
    true_wcde = compute_theoretical_wcde_montecarlo(
        coeffs, selected_func, n_b1g1=6000, n_g2=6000, seed=seed
    )
...
def generate_coeffs_list_parallel(G, n_coeff_sets, seed_base=0):
    ...
    print(f"Submitted {n_coeff_sets} Monte Carlo ground-truth tasks; waiting for Ray...")
...
def compare_adjusters_fine_grain(coeffs_list, n_rep, n_sample, treatment, outcome, all_mediators, adjusters_list):
    ...
    while futures:
        ready, futures = ray.wait(futures, num_returns=min(200, len(futures)), timeout=30.0)
        if ready:
...
if __name__ == '__main__':
    ...
    # Normalize to frozenset to avoid "not in" failures due to type mismatch
    valid_sets = [frozenset(s) for s in valid_sets_raw]
    print(f"All mediators: {sorted(all_mediators)}")
    ...
    print(f"  - Number of repetitions per set: {n_rep}")
    print(f"  - Sample sizes: {n_samples}")
    print(f"  - Using AIPW method to estimate WCDE; counterfactual prediction lookup; joint vs product discriminative density ratio; empirical distribution weighting")
    ...
    a_y_by_idx = {idx: float((coef_tuple[0]['A_Y'])) for idx, coef_tuple in enumerate(coeffs_list)}
    print(f"Mean of coefficient |A_Y|: {avg_a_y:.6f}  (std={std_a_y:.6f})")
    ...
    for n_sample in n_samples:
        ...
        # Cross-sample summary (generate even if only one n)
    ...
    print(f"Total runtime: {int(hours):02d}h:{int(minutes):02d}m:{seconds:.2f}s")
